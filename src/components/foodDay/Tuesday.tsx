...
export function Tuesday() {
  ...
  return (
    <CalendarDIV>
      {foods.length === 0 ? null : (
        <Link to={"dayDetaile"} state={{ foodDate: "화" }}>
          <DaySpan>화</DaySpan>
        </Link>
      )}
      {foods.length === 0
        ...
        : foods.map((food, index) =>
            food.date === "화" ||
            (food.date === "화아" && food.isTwo !== false) ||
            (food.date === "화점" && food.isTwo === true) ? (
              <Link key={index} to={food.date} state={{ foodTime: food.time }}>
                <EachCalendarUL>
                  ...
                  <EachCalendarLI key={food.type}>
                    {food.date === "화" ? food.type : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.mainFood}>
                    {food.date === "화" ? food.mainFood : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood1}>
                    {food.date === "화" ? food.sideFood1 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood2}>
                    {food.date === "화" ? food.sideFood2 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood3}>
                    {food.date === "화" ? food.sideFood3 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.rice}>
                    {food.date === "화" ? food.rice : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.soup}>
                    {food.date === "화" ? food.soup : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.drink}>
                    {food.date === "화" ? food.drink : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.salad}>
                    {food.date === "화" ? food.salad : null}
                  </EachCalendarLI>
                </EachCalendarUL>
              </Link>
            ) : null
