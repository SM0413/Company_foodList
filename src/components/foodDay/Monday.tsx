...
export function Monday() {
  ...
  return (
    <CalendarDIV>
      {foods.length === 0 ? null : (
        <Link to={"dayDetaile"} state={{ foodDate: "월" }}>
          <DaySpan>월</DaySpan>
        </Link>
      )}
      {foods.length === 0
              ...
              <Link key={index} to={food.date} state={{ foodDate: food.date }}>
                <EachCalendarUL>
                  ...
                  <EachCalendarLI key={food.mainFood}>
                    {food.date === "월" ? food.mainFood : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood1}>
                    {food.date === "월" ? food.sideFood1 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood2}>
                    {food.date === "월" ? food.sideFood2 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood3}>
                    {food.date === "월" ? food.sideFood3 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.rice}>
                    {food.date === "월" ? food.rice : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.soup}>
                    {food.date === "월" ? food.soup : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.drink}>
                    {food.date === "월" ? food.drink : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.salad}>
                    {food.date === "월" ? food.salad : null}
                  </EachCalendarLI>
                </EachCalendarUL>
              </Link>
            ) : null
