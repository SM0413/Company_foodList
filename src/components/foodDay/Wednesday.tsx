...
export function Wednesday() {
  ...
  return (
    <CalendarDIV>
      {foods.length === 0 ? null : (
        <Link to={"dayDetaile"} state={{ foodDate: "수" }}>
          <DaySpan>수</DaySpan>
        </Link>
      )}
      {foods.length === 0
        ...
        : foods.map((food, index) =>
            food.date === "수" ||
            (food.date === "수아" && food.isTwo !== false) ||
            (food.date === "수점" && food.isTwo === true) ? (
              <Link key={index} to={food.date} state={{ foodTime: food.time }}>
                <EachCalendarUL>
                  ...
                  <EachCalendarLI key={food.type}>
                    {food.date === "수" ? food.type : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.mainFood}>
                    {food.date === "수" ? food.mainFood : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood1}>
                    {food.date === "수" ? food.sideFood1 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood2}>
                    {food.date === "수" ? food.sideFood2 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.sideFood3}>
                    {food.date === "수" ? food.sideFood3 : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.rice}>
                    {food.date === "수" ? food.rice : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.soup}>
                    {food.date === "수" ? food.soup : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.drink}>
                    {food.date === "수" ? food.drink : null}
                  </EachCalendarLI>
                  <EachCalendarLI key={food.salad}>
                    {food.date === "수" ? food.salad : null}
                  </EachCalendarLI>
                </EachCalendarUL>
              </Link>
            ) : null
